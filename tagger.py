...
model.reload()
# with open('f_eval.pkl', 'wb') as f:
#     cPickle.dump(f_eval, f)

# # Load the parameters and the mappings from disk
# with open('f_eval.pkl', 'rb') as f:
#     f_eval = cPickle.load(f)
    ...
    for line in f_input:
        ...
        if len(line.strip()):
            ...
            for w, y in zip(words, y_preds):
                if y == 'B-PER':
                    text = text + ('%s' % ''.join('%s%s[First_Name]' % (w, opts.delimiter)))
                elif y == 'I-PER':
                    text = text + ('%s' % ''.join('%s%s[Last_Name]' % (w, opts.delimiter)))
                else:
                    text = text + ('%s' % ''.join('%s' % (w)))
                text = text + ' '            
            print(text)
            f_output.write('%s\n' % text)
